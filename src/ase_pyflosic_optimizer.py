...
def flosic_optimize(mode='flosic-scf',atoms=None,charge=0,spin=0,xc='lda,pw',basis='STO-3G',ecp=None,opt='FIRE',maxstep=0.2,label='OPT_FRMORB',fmax=1.5e-4*(Ha/Bohr),steps=1000,max_cycle=300,conv_tol=1e-6,grid=3,df=False,newton=False,use_chk=False,verbose=4,efield=None,ham_sic='HOOOV',dm=None,cart=False,output=None,solvation=None,lmax=10,eta=0.1,lebedev_order=89,rad_sol='vdw',rad_sol_scal=1.0,eps=78.3553,pol=False,n_rad=None,n_ang=None,prune='no'):
    ...
    # rad_sol                   radii for solvation model
    ...
    calc = PYFLOSIC(mode=mode,atoms=atoms,charge=charge,spin=spin,xc=xc,basis=basis,ecp=ecp,max_cycle=max_cycle,conv_tol=conv_tol,grid=grid,df=df,newton=newton,verbose=verbose,efield=efield,ham_sic=ham_sic,use_chk=use_chk,dm=dm,cart=cart,output=output,solvation=solvation,lmax=lmax,eta=eta,lebedev_order=lebedev_order,rad_sol=rad_sol,rad_sol_scal=rad_sol_scal,eps=eps,pol=pol,n_rad=n_rad,n_ang=n_ang,prune=prune)
